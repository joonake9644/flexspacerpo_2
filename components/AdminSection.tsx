...
import { Booking, Program, ProgramApplication, User } from '@/types'
import { httpsCallable } from 'firebase/functions'
import { functions } from '@/utils/firebase-functions'
import { PlusCircle, Edit, Trash2, UserCheck, Calendar as CalendarIcon } from 'lucide-react'
...
export default function AdminSection({ bookings, setBookings, applications, setApplications, programs, setPrograms }: AdminSectionProps) {
  ...
  const handleBookingAction = useCallback(async (bookingId: string, action: 'approve'|'reject') => {
    try {
      await updateReservationStatus({ reservationId: bookingId, status: action === 'approve' ? 'approved' : 'rejected' })
      setBookings(prev => prev.map(b => b.id === bookingId ? { ...b, status: action === 'approve' ? 'approved' : 'rejected' } : b))
      showNotification(`대관이 ${action === 'approve' ? '승인' : '거절'}되었습니다.`, 'success')
    } catch (e: any) {
      showNotification(`처리 실패: ${e?.message ?? '알 수 없는 오류'}`, 'error')
    }
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3 mb-3">
            <div className="w-10 h-10 bg-orange-50 rounded-lg flex items-center justify-center"><CalendarIcon className="w-5 h-5 text-orange-600"/></div>
            <h3 className="text-lg font-semibold text-gray-900">진행중인 대관 신청 ({pendingBookings.length})</h3>
          </div>
          <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
            {pendingBookings.length>0 ? pendingBookings.map(b => (
              <PendingBookingItem key={b.id} b={b} onAction={handleBookingAction} />
            )) : <p className="text-gray-500 text-center py-4">진행중인 신청이 없습니다.</p>}
          </div>
        </div>

        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center space-x-3 mb-3">
            <div className="w-10 h-10 bg-purple-50 rounded-lg flex items-center justify-center"><UserCheck className="w-5 h-5 text-purple-600"/></div>
            <h3 className="text-lg font-semibold text-gray-900">진행중인 프로그램 신청 ({pendingApplications.length})</h3>
          </div>
          <div className="space-y-3 max-h-96 overflow-y-auto pr-2">
            {pendingApplications.length>0 ? pendingApplications.map(a => (
              <div key={a.id} className="p-4 bg-gray-50 rounded-xl transition-shadow hover:shadow-md">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-gray-900">신청 ID: {a.id}</p>
                    <p className="text-sm text-gray-600 mt-1">상태: {a.status}</p>
                  </div>
                </div>
              </div>
            )) : <p className="text-gray-500 text-center py-4">진행중인 신청이 없습니다.</p>}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => { setProgramToEdit(null); setProgramForm(emptyProgram); setIsProgramModalOpen(true) }} className="flex items-center gap-2 text-sm font-medium text-purple-600 hover:text-purple-800 px-4 py-2 rounded-lg hover:bg-purple-50">
